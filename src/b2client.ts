...
const TOKEN_TTL_MS = 23 * 60 * 60 * 1000; // 23h - docs doesn't state expiry
...
export async function listBuckets(): Promise<B2Bucket[]> {
  ...
  if (!allowed.capabilities.some((c) => c === "listBuckets")) {
    throw Error(
      "You don't have permission to list buckets. Please check your application key capabilities.",
    );
  }
  const payload: Record<string, string> = { accountId: auth.accountId };
  if (allowed.buckets !== null && allowed.buckets.length === 1) {
    payload.bucketId = allowed.buckets[0].id;
  }
...
export async function listFiles(
  ...
  const response = await axios.get<ListFilesResult>(
    `${apiUrl}/b2api/v4/b2_list_file_names`,
...
export async function downloadFile(
  ...
  const auth = await authorize();
  const { downloadUrl, allowed } = auth.storage;
  const encodedName = encodeURIComponent(fileName).replace(/%2F/g, "/");
...
export async function getFileInfo(fileId: string): Promise<B2FileInfo> {
  ...
  const response = await axios.get<B2FileInfo>(
    `${apiUrl}/b2api/v4/b2_get_file_info`,
    {
      headers: { Authorization: auth.token },
      params: { fileId },
      timeout: 30000,
...
export function handleApiError(error: unknown): string {
  if (error instanceof AxiosError) {
    ...
    switch (status) {
      case 400:
        return `Error (${code}): ${message}. Check parameter values.`;
      case 401:
        return `Error (${code}): Auth failed — ${message}. Check B2_APPLICATION_KEY_ID and B2_APPLICATION_KEY.`;
      case 403:
        return `Error (${code}): Access denied — ${message}. Check application key capabilities.`;
      case 404:
        return `Error: Not found — ${message}.`;
      case 429:
        return `Error: Rate limit exceeded. Wait before retrying.`;
      case 503:
        return `Error: Service unavailable. Retry after a moment.`;
      default:
